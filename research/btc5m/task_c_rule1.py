...
------------------------------------------------------------------------------
{
  "meta": {
      ...
      "complete":  bool,         # False in the partial snapshots written after
                                 # each family, True only on the final save
  },
...
def iid_busts(acc, n):
    """
    Busts a 3-rung ladder produces if losses are independent.

    The ladder resets on a win or on the third loss, so a cycle has expected
    length 1+q+q^2 and busts with probability q^3. This is the number every
    variant must be judged against: a variant that "cuts busts" purely by taking
    fewer bets, or purely by being more accurate, has discovered nothing new.
    """
    q = 1 - acc / 100.0
    return n * q ** 3 / (1 + q + q * q)


def lag1(sig):
    """P(win | previous signal won) vs P(win | previous signal lost)."""
    aw = an = bw = bn = 0
    for a, b in zip(sig, sig[1:]):
        if a[3]:
            ...
            bn += 1
            bw += b[3]
    return (aw / an * 100 if an else float("nan"), an,
            bw / bn * 100 if bn else float("nan"), bn)
...
def report(d=None):
    d = d or engine.restore("task_c.pkl")
    m, p1, V, sh = d["meta"], d["part1"], d["variants"], d["shuffle"]
    bz = m["bonf_z"]

    # part 1 detail that needs the chart again (cheap: one run_many)
    cand = engine.last_year(engine.load())
    fired_all = engine.run_many(m["pool_members"], cand)
    seq = []
    for i, ts, f, nxt in fired_all:
        s = set(f.values())
        if len(s) != 1 or nxt == 0:
            continue
        side = s.pop()
        seq.append(("rule1" in f, (side == "up") == (nxt > 0)))
    share = sum(1 for a, _ in seq if a) / len(seq)
    # how sticky is "this window is a rule-1 window"?
    cont = sum(1 for a, b in zip(seq, seq[1:]) if a[0] and b[0])
    n_a = sum(1 for a in seq[:-1] if a[0])
    sticky = cont / n_a * 100 if n_a else float("nan")

    b = p1["pool"]["busts"]
    exp_all1 = b * share ** 3
    exp_any1 = b * (1 - (1 - share) ** 3)

    base = V["baseline"]
    lw, ln_, ll, lnl = lag1(base["sig"])

    rows = [(n, r) for n, r in V.items() if r["n"] >= m["min_n"]]

    def clears_shuffle(n, r):
        s = sh["per_variant"].get(n)
        return s is not None and r["acc"] > s["max"]

    L = []
    A = L.append
    A("# قانون ۱ — آیا شکستِ ۲۰ کندلی همان چیزی است که نردبان را می‌ترکاند؟\n")
    A(f"داده: {m['candles']:,} کندلِ ۵ دقیقه‌ایِ واقعی، از {m['from']} تا {m['to']} "
      f"({m['days']:.0f} روز). پایه ${m['base']:.0f}، نردبانِ {m['rungs']} پله، "
      "تسویهٔ close-to-close، یک شرط در هر پنجره. همهٔ اعداد از `engine.py` "
      "می‌آیند و با `task_c_rule1.py` دوباره تولید می‌شوند.\n")

    A("\n## پاسخِ مستقیم به باور\n")
    A("**نه — قانون ۱ عاملِ انفجارها نیست. کم‌انفجارترین عضوِ استخر است.**\n")
    A(f"- نرخِ انفجارِ قانون ۱ به‌تنهایی **{fa(p1['rule1_alone']['bust_rate'])}%** است، "
      f"نرخِ کلِ استخر **{fa(p1['pool']['bust_rate'])}%**، و نرخِ همان استخر وقتی "
      f"پنجره‌های قانون ۱ را بیرون بیندازیم **{fa(p1['pool_skip1']['bust_rate'])}%** — "
      "یعنی نیمهٔ *بدونِ* قانون ۱ بیشتر می‌ترکد، نه کمتر.")
    A(f"- دقتِ قانون ۱ **{fa(p1['rule1_alone']['acc'])}%** است، بالاتر از میانگینِ "
      f"استخر (**{fa(p1['pool']['acc'])}%**).")
    A(f"- اگر قانون ۱ را کاملاً از استخر حذف کنیم: سیگنال‌ها از "
      f"{p1['pool']['n']:,} به {p1['pool_no1']['n']:,} می‌رسند "
      f"(−{fa((1-p1['pool_no1']['n']/p1['pool']['n'])*100,0)}%)، انفجارها از "
      f"{p1['pool']['busts']:,} به {p1['pool_no1']['busts']:,} "
      f"(−{fa((1-p1['pool_no1']['busts']/p1['pool']['busts'])*100,0)}%)، و سود از "
      f"${p1['pool']['pnl']:,.0f} به ${p1['pool_no1']['pnl']:,.0f}. "
      "انفجارها دقیقاً به همان نسبتِ سیگنال‌ها کم می‌شوند — نه بیشتر.\n")

    A("\n## ولی چرا این‌طور به نظر می‌رسد؟ (اینجا حق با توست)\n")
    A(f"قانون ۱ در **{fa(share*100,1)}%** از پنجره‌های استخر حاضر است، ولی سیگنال‌هایش "
      "خوشه‌ای می‌آیند: اگر این پنجره مالِ قانون ۱ باشد، احتمالِ اینکه پنجرهٔ بعدی هم "
      f"مالِ قانون ۱ باشد **{fa(sticky,1)}%** است. نتیجه:\n")
    A(f"- از {p1['busts_total']:,} انفجارِ استخر، **{p1['busts_all1']:,}** تا "
      f"({fa(p1['busts_all1']/p1['busts_total']*100,0)}%) هر سه پله‌شان قانون ۱ بوده — "
      f"اگر پنجره‌ها تصادفی مخلوط بودند فقط حدود {exp_all1:.0f} تا انتظار می‌رفت.")
    A(f"- {p1['busts_any1']:,} انفجار حداقل یک پلهٔ قانون ۱ دارند (انتظارِ تصادفی "
      f"≈ {exp_any1:.0f}) و **{p1['busts_no1']:,}** انفجار هیچ ربطی به قانون ۱ ندارد.")
    A(f"- از {p1['streak6_total']} رشتهٔ باختِ ۶ تایی یا بلندتر، "
      f"{p1['streak6_any1']} تا حداقل یک پنجرهٔ قانون ۱ دارند و "
      f"**{p1['streak6_all1']}** تا کاملاً از قانون ۱ ساخته شده‌اند.\n")
    A("پس حسِ تو درست است — انفجارهای «سه‌تاییِ پشتِ سرِ هم» واقعاً بیشتر از حدِ "
      "تصادف یک‌دست از قانون ۱ ساخته می‌شوند. ولی این به‌خاطرِ بدتر بودنِ قانون ۱ "
      "نیست؛ به‌خاطرِ این است که پشتِ سرِ هم شلیک می‌کند، پس وقتی می‌بازد "
      "پیاپی می‌بازد و انفجارش یک‌دست دیده می‌شود.\n")

    A("\n## حسابی که همه‌چیز را تعیین می‌کند\n")
    A("اگر باخت‌ها مستقل باشند، نردبانِ ۳ پله در هر سیگنال با احتمالِ "
      "`q³/(1+q+q²)` می‌ترکد (q = نرخِ باخت). برای دقتِ پایه یعنی "
      f"**{fa(iid_busts(base['acc'],100),2)}%** و اندازه‌گیریِ واقعی "
      f"**{fa(base['bust_rate'],2)}%** است. عملاً یکی هستند.\n")
    A("نتیجهٔ سختِ این حساب: **هیچ فیلترِ زمانی نمی‌تواند نرخِ انفجار را کم کند.** "
      "فقط دو راه هست — دقت را بالا ببری (نرخ پایین می‌آید)، یا کمتر شرط ببندی "
      "(تعداد پایین می‌آید، نرخ ثابت می‌ماند).\n")
    dz = ((ll - lw) / 100) / math.sqrt(0.25 / ln_ + 0.25 / lnl)
    A(f"و باخت‌های قانون ۱ اصلاً به هم نمی‌چسبند — برعکس: بعد از یک **برد** دقتِ "
      f"سیگنالِ بعدی **{fa(lw)}%** است (n={ln_:,}) و بعد از یک **باخت** "
      f"**{fa(ll)}%** (n={lnl:,}). یعنی سیگنالِ بعد از باخت اگر تفاوتی داشته باشد "
      f"کمی *بهتر* است، نه بدتر (z={fa(dz)}، در حدِ نویز). به همین دلیل هیچ نسخه‌ای "
      "از «بعد از باخت نگیر» نمی‌تواند کار کند — دارد سیگنال‌های خوب را حذف می‌کند.\n")

    A("\n## ایدهٔ خودت: «بعد از باخت پشتِ سرِ هم نگیر»\n")
    A("| حالت | n | دقت | بازهٔ ۹۵٪ | انفجار | نرخِ انفجار | بلندترین رشته | سود | کفِ مسیر | برتری در بلوکِ ماهانه |")
    A("|---|---|---|---|---|---|---|---|---|---|")
    order = ["baseline", "after_loss_skip1", "after_loss_skip2", "after_loss_skip3",
             "after_loss_skip5", "after_loss_wait5c", "after_loss_wait10c",
             "after_loss_wait20c", "after_loss_wait60c", "after_loss_reenter",
             "pause_day_after_bust"]
    for n in order:
        r = V[n]
        A(f"| {r['desc']} | {r['n']:,} | {fa(r['acc'])}% | "
          f"{fa(r['lo'],1)}–{fa(r['hi'],1)} | {r['busts']:,} | {fa(r['bust_rate'])}% | "
          f"{r['max_streak']} | ${r['pnl']:,.0f} | ${r['path_low']:,.0f} | "
          f"{r['block_lift']:+.2f} |")
    A("\n**هر ده حالت دقت را پایین می‌آورند یا دست‌نخورده می‌گذارند، و نرخِ انفجار "
      "را کم نمی‌کنند.** تعدادِ انفجار فقط چون تعدادِ شرط کم شده پایین آمده. "
      "این دقیقاً همان چیزی است که از مستقل بودنِ باخت‌ها انتظار می‌رود.\n")

    # ---- family by family --------------------------------------------------
    FAM = [
        ("3-cluster", "۳) سرکوبِ خوشه",
         "برخلافِ انتظار، **اولینِ** شکستِ هر خوشه بدترینِ آن‌هاست و شکستِ دوم و "
         "سوم بهترند. «فقط اولی را بگیر» ایدهٔ اشتباهی است."),
        ("4-lookback", "۴) جاروبِ پنجرهٔ سطح",
         "دقت بینِ 10 و 100 کندل تقریباً صاف است (53.7٪ تا 54.5٪). پنجرهٔ 20 که "
         "الان کار می‌کند بد نیست؛ 40 و 50 نصفِ سیگنال با همان دقت می‌دهند."),
        ("5-vol", "۵) فیلترِ نوسان، شاملِ حالتِ وارونه",
         "فیلتر واقعی است و جهتش هم درست: هر چه آستانه بالاتر، دقت بهتر. "
         "حالتِ **وارونه** (نوسانِ در حالِ انقباض) در همهٔ آستانه‌ها بدتر از پایه "
         "است — تا 51.8٪ پایین می‌آید. یعنی شکست در نوسانِ کم را باید انداخت، "
         "نه اینکه فقط آن را گرفت."),
        ("6-direction", "۶) عدمِ تقارنِ جهت",
         "شکستِ سقف 54.70٪ و شکستِ کف 53.95٪ — اختلاف 0.75 واحد با n≈5,500 در هر "
         "طرف، که در حدِ نویز است. قانون ۱ برخلافِ قانون ۶ یک‌طرفه نیست."),
        ("7-depth", "۷) عمقِ شکست",
         "یکی از دو یافتهٔ واقعیِ این جاروب. هر چه کلوز عمیق‌تر از سطح رد شود، "
         "دقت بالاتر می‌رود و این رابطه یکنواخت است: 54.8٪ → 55.4٪ → 55.6٪ → 56.0٪."),
        ("8-confirm", "۸) تأییدِ قانونِ دیگر",
         "یافتهٔ دومِ واقعی. همراهیِ قانون ۲ بهترین است. توجه: ستونِ «هم‌جهت» "
         "دقیقاً برابرِ ستونِ ساده است، چون قانون‌ها هرگز مخالفِ هم نمی‌شوند."),
        ("9-veto", "۹) وتوی قانونِ دیگر",
         "کاملاً خالی — هر شش حالت عددِ پایه را تکرار می‌کنند. توضیح در بخشِ آخر."),
        ("10-calendar", "۱۰) ساعت و روزِ هفته (تهران)",
         "شنبه و پنجشنبه و بازهٔ 20:00–24:00 بهتر از بقیه‌اند، ولی هیچ سازوکاری "
         "پشتشان نیست و n هر کدام کوچک است."),
        ("11-highlow", "۱۱) شکست روی های/لو به‌جای کلوز",
         "مهم: اگر **سطح** را از های/لو بگیریم ولی **شکست** را با کلوز بسنجیم، "
         "دقت به 55.40٪ می‌رسد (از 54.32٪) با ۴۲٪ سیگنالِ کمتر. ولی اگر شکست را "
         "هم با های/لوِ خودِ کندل بسنجیم، دقت به 53.54٪ می‌افتد. یعنی کارِ ربات "
         "(شکست با کلوز) درست است و فقط سطحش را می‌شود عوض کرد."),
    ]
    A("\n## خانواده به خانواده\n")
    for g, title, note in FAM:
        sub = [(n, r) for n, r in V.items() if r["group"] == g]
        if not sub:
            continue
        A(f"\n### {title}\n")
        A("| حالت | n | دقت | بازهٔ ۹۵٪ | z | انفجار | نرخِ انفجار | سود | بلوکِ ماهانه |")
        A("|---|---|---|---|---|---|---|---|---|")
        A(f"| _پایه_ | {base['n']:,} | {fa(base['acc'])}% | "
          f"{fa(base['lo'],1)}–{fa(base['hi'],1)} | {fa(base['z'])} | "
          f"{base['busts']:,} | {fa(base['bust_rate'])}% | ${base['pnl']:,.0f} | — |")
        for n, r in sorted(sub, key=lambda x: -x[1]["acc"]):
            A(f"| {n} | {r['n']:,} | {fa(r['acc'])}% | "
              f"{fa(r['lo'],1)}–{fa(r['hi'],1)} | {fa(r['z'])} | {r['busts']:,} | "
              f"{fa(r['bust_rate'])}% | ${r['pnl']:,.0f} | {r['block_lift']:+.2f} |")
        A(f"\n{note}\n")

    A("\n## رتبه‌بندیِ کاملِ حالت‌ها بر اساسِ کمترین انفجار\n")
    A(f"فقط حالت‌هایی با n ≥ {m['min_n']}. ستونِ «انتظار» تعدادِ انفجاری است که "
      "از روی دقت و تعدادِ همان حالت پیش‌بینی می‌شود؛ برابر بودنِ آن با ستونِ "
      "«انفجار» یعنی آن حالت هیچ ساختارِ تازه‌ای پیدا نکرده و فقط کمتر شرط بسته است.\n")
    A("| # | حالت | گروه | n | در روز | دقت | بازهٔ ۹۵٪ | z | انفجار | انتظار | نرخ | رشته | سود | کفِ مسیر | train | test | بلوک |")
    A("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for k, (n, r) in enumerate(sorted(rows, key=lambda x: x[1]["busts"]), 1):
        A(f"| {k} | {n} | {r['group']} | {r['n']:,} | {fa(r['per_day'],1)} | "
          f"{fa(r['acc'])}% | {fa(r['lo'],1)}–{fa(r['hi'],1)} | {fa(r['z'])} | "
          f"{r['busts']:,} | {iid_busts(r['acc'], r['n']):.0f} | "
          f"{fa(r['bust_rate'])}% | {r['max_streak']} | ${r['pnl']:,.0f} | "
          f"${r['path_low']:,.0f} | {fa(r['train']['acc'])}% | "
          f"{fa(r['test']['acc'])}% | {r['block_lift']:+.2f} |")

    A("\n## چه چیزی واقعاً از سدِ کنترل‌ها رد شد\n")
    A(f"K = {m['K']} حالت آزموده شد، پس سدِ بونفرونی z ≥ **{fa(bz,3)}** است، نه ۱٫۹۶.\n")
    A("پنج کنترل، همه با هم:\n")
    A(f"1. z کلِ سال ≥ {fa(bz,3)} (بونفرونی برای K={m['K']}).")
    A("2. هر دو نیمهٔ train و test بالای ۵۰٪.")
    A(f"3. نیمهٔ test از نیمهٔ test خودِ پایه ({fa(base['test']['acc'])}%) بالاتر "
      "باشد — چون نیمهٔ دومِ این سال ذاتاً بیشتر فید می‌کند و مقایسه باید "
      "درونِ همان دوره باشد.")
    A("4. دقتِ واقعی از **بیشینهٔ** دقتی که همان حالت روی برچسبِ درهم گرفت بالاتر باشد.")
    A("5. برتری در بلوکِ ماهانه نسبت به پایه بیشتر از +0.5 واحد.\n")
    A("| حالت | n | دقت | z | test − test پایه | سقفِ برچسبِ درهم | بلوکِ ماهانه | حکم |")
    A("|---|---|---|---|---|---|---|---|")
    surv, near = [], []
    for n, r in sorted(rows, key=lambda x: -x[1]["z"]):
        ok_z = r["z"] >= bz
        ok_tt = r["train"]["acc"] > 50 and r["test"]["acc"] > 50
        ok_oos = r["test"]["acc"] > base["test"]["acc"]
        ok_sh = clears_shuffle(n, r)
        ok_bl = r["block_lift"] > 0.5
        if not (ok_z and ok_tt and ok_sh and ok_bl):
            continue
        s = sh["per_variant"][n]
        d_oos = r["test"]["acc"] - base["test"]["acc"]
        (surv if ok_oos else near).append((n, r))
        A(f"| {n} | {r['n']:,} | {fa(r['acc'])}% | {fa(r['z'])} | "
          f"{d_oos:+.2f} {'✅' if ok_oos else '❌'} | {fa(s['max'])}% ✅ | "
          f"{r['block_lift']:+.2f} ✅ | "
          f"{'**قبول**' if ok_oos else 'مردود در test'} |")
    A(f"\n**{len(surv)} حالت هر پنج کنترل را با هم رد کرد.** "
      f"{len(near)} حالتِ دیگر چهار کنترلِ اول را رد کردند ولی در نیمهٔ test از "
      "پایه جلو نزدند.\n")
    A("دو موردِ مهم که در همین‌جا رد شدند و نباید گولشان را خورد:\n")
    cg = V.get("combo_ge2_first")
    if cg:
        A(f"- `combo_ge2_first` با {fa(cg['acc'])}% بالاترین دقتِ کلِ جاروب را دارد، "
          f"ولی train {fa(cg['train']['acc'])}% و test {fa(cg['test']['acc'])}% — "
          "یعنی در نیمهٔ دوم فرو ریخته. و دقیقاً همین حالت است که روی برچسبِ درهم "
          f"هم بالاترین دقتِ تصادفی ({fa(sh['best_acc'])}%) را ساخت. با n="
          f"{cg['n']:,} این یک یافته نیست، یک آدرسِ غلط است.")
    A("- حالت‌های تقویمی (`dow_*`، `tehran_*`) از کنترل‌های عددی رد می‌شوند ولی "
      "هیچ سازوکاری پشتشان نیست؛ در بخشِ آخر توضیح داده شده.\n")

    A("\n## کنترلِ برچسبِ درهم\n")
    A(f"برچسبِ بالا/پایینِ کلِ سال {sh['reps']} بار درهم شد و همهٔ "
      f"{len(sh['per_variant'])} حالتِ n ≥ {m['min_n']} دوباره امتیاز گرفتند "
      "(حالت‌های وابسته به نتیجه از نو اجرا شدند، نه اینکه فقط یک لیستِ بولی "
      "جابه‌جا شود).\n")
    A(f"- بهترین دقتی که *هر* حالت روی برچسبِ تصادفی گرفت: "
      f"**{fa(sh['best_acc'])}%** (`{sh['best_variant']}`)")
    A(f"- توزیعِ بیشینهٔ هر جاروب: میانگین {fa(sh['mean'])}% · "
      f"p95 {fa(sh['p95'])}% · p99 {fa(sh['p99'])}%\n")
    A("یعنی هر دقتی زیرِ حدودِ ۵۵٪ روی یک نمونهٔ کوچک را همین جاروب می‌تواند "
      "از دلِ نویز بیرون بکشد. سقفِ درهمِ *هر حالت به‌طورِ جداگانه* در جدولِ "
      "بالا آمده و ملاکِ سخت‌گیرانه‌تر همان است.\n")

    A("\n## پیشنهادِ نهایی — بهترین ۵ حالت\n")
    A("از میانِ حالت‌هایی که هر پنج کنترل را رد کردند، تقویمی‌ها کنار گذاشته "
      "شده‌اند (سازوکار ندارند) و بقیه بر اساسِ نرخِ انفجار مرتب شده‌اند. "
      "`_side` کنار گذاشته شده چون با نسخهٔ ساده‌اش یکی است.\n")
    best = [(n, r) for n, r in surv
            if r["group"] != "10-calendar" and not n.endswith("_side")]
    best = sorted(best, key=lambda x: x[1]["bust_rate"])[:5]
    A("| حالت | توضیح | n | در روز | دقت | بازهٔ ۹۵٪ | انفجار | نرخِ انفجار | "
      "بلندترین رشته | سود | کفِ مسیر |")
    A("|---|---|---|---|---|---|---|---|---|---|---|")
    for n, r in best:
        A(f"| {n} | {r['desc']} | {r['n']:,} | {fa(r['per_day'],1)} | "
          f"{fa(r['acc'])}% | {fa(r['lo'],1)}–{fa(r['hi'],1)} | {r['busts']:,} | "
          f"{fa(r['bust_rate'])}% | {r['max_streak']} | ${r['pnl']:,.0f} | "
          f"${r['path_low']:,.0f} |")
    A("\nهر پنج تا نسبت به پایه هم انفجارِ کمتری دارند و هم نرخِ انفجارِ کمتر — "
      "یعنی برخلافِ ایدهٔ «بعد از باخت نگیر»، اینجا واقعاً کیفیت بالا رفته، نه "
      "فقط کمیت پایین آمده. ولی حواست باشد سودِ دلاری هم پایین می‌آید، چون "
      "شرط‌های کمتری بسته می‌شود.\n")
    r2 = V["and_rule2"]
    A(f"یک موردِ نزدیک که بیرون ماند: `and_rule2` (قانون ۱ همراهِ قانون ۲) با "
      f"{fa(r2['acc'])}% و n={r2['n']:,} بالاترین برتریِ بلوکِ ماهانه در بینِ "
      f"حالت‌های بزرگ را دارد ({r2['block_lift']:+.2f}) و train/test آن هم از همه "
      f"پایدارتر است ({fa(r2['train']['acc'])}% / {fa(r2['test']['acc'])}%). فقط "
      f"چون نیمهٔ test آن {r2['test']['acc']-base['test']['acc']:+.2f} واحد از "
      "نیمهٔ test پایه پایین‌تر افتاد از فهرست بیرون ماند. اگر می‌خواهی یکی را "
      "امتحان کنی، این و `depth_1.0` دو گزینهٔ اولند.\n")

    A("\n## چیزهایی که نتوانستم تأیید کنم\n")
    A("- **گروهِ «وتوی قانونِ دیگر» عملاً خالی است.** در "
      "۱۲٬۸۷۶ پنجره‌ای که قانون ۱ همراهِ یکی از قانون‌های ۲/۳/۵/۶/۷/طلایی شلیک "
      "کرد، **صفر** بار جهتِ مخالف دادند. هر شش قانون فیدِ همان حرکتِ اخیرند، "
      "پس ساختاراً نمی‌توانند مخالفِ هم باشند. هر شش حالتِ وتو دقیقاً برابرِ پایه شد.")
    A("- هیچ دادهٔ دفترِ سفارشِ پلی‌مارکت وجود ندارد. همهٔ سودها با فرضِ قیمتِ ۵۰ "
      "سنت حساب شده‌اند؛ بالای ۵۵ سنت این لبه‌ها می‌میرند.")
    A("- نیمهٔ آزمونِ این سال ذاتاً بیشتر فید می‌کند (پایه: train "
      f"{fa(base['train']['acc'])}% → test {fa(base['test']['acc'])}%). به همین "
      "دلیل ستونِ «بلوکِ ماهانه» — نه ستونِ test — ملاکِ واقعیِ برتری است.")
    A("- حالت‌های تقویمی (روزِ هفته، ساعتِ تهران) با اینکه از کنترل‌ها رد می‌شوند، "
      "هیچ سازوکاری پشتشان نیست و n هر کدام حدودِ ۱٬۰۰۰ تا ۳٬۰۰۰ است. "
      "به آن‌ها به چشمِ یافته نگاه نکن.")

    path = os.path.join(HERE, "reports", "task-c-rule1.md")
