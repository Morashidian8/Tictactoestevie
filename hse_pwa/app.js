...
let currentCategory = null; // دسته‌بندی فعلی در حال تکمیل
...
function saveHistory(item){ try{ const h=getHistory(); h.unshift({...item, timestamp: new Date().toLocaleString('fa-IR')}); if(h.length>3) h.pop(); localStorage.setItem(HISTORY_KEY, JSON.stringify(h)); }catch(e){} }
...
// =================== انتخاب دسته‌بندی ===================
// شمارش ردیف‌های پاسخ‌داده‌شدهٔ یک دسته
function categoryProgress(section){
  const items = ITEMS.filter(it => it.section === section);
  const done = items.filter(it => state.answers[it.row] && state.answers[it.row].status).length;
  return { total: items.length, done, complete: items.length > 0 && done === items.length };
}
// آیا همهٔ ردیف‌های همهٔ دسته‌بندی‌ها پاسخ داده شده‌اند؟
function allItemsComplete(){
  return ITEMS.every(it => state.answers[it.row] && state.answers[it.row].status);
}
...
function showCategorySelection(){
  ...
  CHECKLIST.sections.forEach((section) => {
    const p = categoryProgress(section);
    if(p.complete) completedCats++;

    const div = document.createElement("div");
    div.style.cssText = "background:#f8fafc;border:1.5px solid #e5e7eb;border-radius:10px;padding:14px;margin-bottom:10px;cursor:pointer;transition:0.15s;display:flex;align-items:center;gap:12px;";
    if(p.complete){ div.style.background = "#d1fae5"; div.style.borderColor = "#2e8b57"; }
    else if(p.done > 0){ div.style.background = "#fef3c7"; div.style.borderColor = "#bf8f00"; }

    const icon = p.complete ? "✅" : (p.done > 0 ? "🔵" : "⚪");
    div.innerHTML = `
      <div style="font-size:1.4rem;">${icon}</div>
      <div style="flex:1;">
        <div style="font-weight:bold;color:#1f4e78;">${section}</div>
        <div style="font-size:0.85rem;color:#666;">${toFa(p.done)} از ${toFa(p.total)} تکمیل شد</div>
      </div>
      <div style="color:#2e75b6;font-weight:bold;font-size:0.9rem;">${p.complete ? "ویرایش" : "ورود ◀"}</div>
    `;
    div.onclick = () => {
      currentCategory = section;
      filteredItems = ITEMS.filter(it => it.section === section);
      idx = 0;
      show("wizardScreen");
      renderItem();
    };
  ...
  // نوار پیشرفت کلی
  const totalCats = CHECKLIST.sections.length;
  const pct = totalCats ? Math.round(completedCats / totalCats * 100) : 0;
  $("#catProgressBar").style.width = pct + "%";
  $("#catProgressText").textContent = `${toFa(completedCats)} از ${toFa(totalCats)} دسته‌بندی تکمیل شد`;

  // فعال/غیرفعال کردن دکمهٔ پایان
  const finishBtn = $("#finishAllBtn");
  const ready = completedCats === totalCats;
  finishBtn.disabled = !ready;
  finishBtn.style.opacity = ready ? "1" : "0.5";
...
async function init(){
  ...
  if(resumeBtn && saved && saved.answers && Object.keys(saved.answers).length){
  ...
  // categoryScreen: پایان و دریافت گزارش — فقط وقتی همهٔ دسته‌بندی‌ها تکمیل شده‌اند
  $("#finishAllBtn").onclick=()=>{
    if(!allItemsComplete()){
      toast("لطفاً ابتدا همهٔ دسته‌بندی‌ها را تکمیل کنید");
      return;
    }
    filteredItems = [...ITEMS]; // گزارش روی همهٔ ردیف‌ها
    showFinish();
  };

  $("#historyBtn").onclick=showHistory;
  $("#backFromHistory").onclick=()=>show("startScreen");
  $("#prevBtn").onclick=()=>{ saveCurrent();
    if(idx>0){ idx--; renderItem(); }
    else if(completionMode === "category"){ showCategorySelection(); } };
...
function renderItem(){
  ...
  if(isLast){
    $("#nextBtn").textContent = completionMode === "category" ? "تکمیل این دسته ✓" : "پایان و گزارش ✓";
  } else {
    $("#nextBtn").textContent = "بعدی ▶";
  }
  // در حالت دسته‌بندی، ردیف اول دکمهٔ برگشت به لیست دسته‌بندی‌ها را نشان می‌دهد
  if(idx===0 && completionMode === "category"){
    $("#prevBtn").style.visibility = "visible";
    $("#prevBtn").textContent = "◀ دسته‌بندی‌ها";
  } else {
...
function onNext(){ const row=filteredItems[idx].row, a=state.answers[row];
  ...
  if(idx === filteredItems.length - 1){
    ...
    if(completionMode === "category"){
      // این دسته تمام شد — برگرد به لیست دسته‌بندی‌ها (خروجی فقط پس از تکمیل همه)
      toast("این دسته‌بندی تکمیل شد ✓");
      showCategorySelection();
      return;
    }
    showFinish();
