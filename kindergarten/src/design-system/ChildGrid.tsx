...
export type ChildGridItem = {
  ...
  napLogged?: boolean
  /**
   * امروز تحویلش با کسِ دیگری است — بخش ۵.۸.
   *
   * فقط نامِ تحویل‌گیرنده، نه جزئیات. متن کامل در شیت تحویل می‌آید،
   * همان لحظه‌ای که مربی واقعاً تصمیم می‌گیرد. اینجا فقط باید بگوید
   * «این یکی فرق دارد» تا مربی غافلگیر نشود.
   *
   * «چه کسی می‌آورد» عمداً نیست: مربی با آن هیچ کار متفاوتی نمی‌کند،
   * و هر خطِ بی‌کنش، خطِ باکنشِ کنارش را کم‌رنگ می‌کند.
   */
...
function GridCell({
  ...
  return (
    ...
    >
      {/*
        چسبِ رنگیِ بالای کارت — تزئین، و فقط تزئین.
        
        رنگش از شناسه کودک می‌آید و در شبکه می‌چرخد، پس هیچ معنایی حمل
        نمی‌کند. **بدنهٔ کارت** جای معناست و دست‌نخورده ماند: تینتِ
        انبه‌ای یعنی آلرژی و مرزِ آجری یعنی بی‌خبر (بخش ۷.۱ و ۵.۳).
        اگر رنگِ بدنه هم می‌چرخید، همان دو پیام خوانده نمی‌شدند.
      */}
      <span className={`${styles.tape} ${styles[`tape${tapeOf(item.id)}`]}`} aria-hidden />
      ...
      */}
      ...
      {/*
        ...
        نامِ تحویل‌گیرنده، نه برچسب «استثنا»: «عمو محمد» به مربی
        می‌گوید چه کسی را انتظار بکشد؛ «استثنا» فقط می‌گوید یک چیزی
        هست و باید جایی دنبالش بگردد.
      */}
      {item.pickupBy ? <span className={styles.pickup}>{item.pickupBy}</span> : null}
...
/**
 * رنگِ چسبِ کارت، از شناسه کودک.
 *
 * از شناسه و نه از جایگاه در فهرست: جایگاه با هر ورود و خروج عوض
 * می‌شود و کارتِ یک کودک هر بار رنگ دیگری می‌گرفت. تزئینی که هر بار
 * می‌پرد، دیده می‌شود — و همان چیزی است که نباید دیده شود.
 */
function tapeOf(id: string): 'A' | 'B' | 'C' | 'D' | 'E' {
  let sum = 0
  for (let i = 0; i < id.length; i += 1) sum += id.charCodeAt(i)
  return (['A', 'B', 'C', 'D', 'E'] as const)[sum % 5]!
}
